...
int write_image_to_server(SSL *ssl, const unsigned char *image_data, size_t image_size) {
  int bytes_sent = 0;
  while (bytes_sent < image_size) {
    int bytes_to_send = SSL_write(ssl, image_data + bytes_sent, image_size - bytes_sent);
    if (bytes_to_send <= 0) {
      return -1;
    }
...
int send_file_to_server(char *filedir)
{
	...
	cJSON_AddNumberToObject(json, "user_id", 3);
	cJSON_AddNumberToObject(json, "chat_id", 4);
	cJSON_AddStringToObject(json, "message", "message");
	cJSON_AddStringToObject(json, "file", buffer);
	cJSON_AddStringToObject(json, "file_name", "hog.png");
	cJSON_AddNumberToObject(json, "time", 1200);
	char *json_str = cJSON_PrintUnformatted(json);

	SSL_write(info->ssl, json_str, mx_strlen(json_str));

	write_image_to_server(info->ssl, buffer, fsize);
