...
int db_init()
{
    ...
    if (sqlite3_exec(db, "INSERT INTO files (filename, file_size, file_path, time) VALUES('default_avatar', 14171, 'Server/db/data/default_avatar.png', 0); ", NULL, 0, &error) != SQLITE_OK)
    {
        mx_logs(error, LOG_ERROR);
        sqlite3_free(error);
        return 1;
